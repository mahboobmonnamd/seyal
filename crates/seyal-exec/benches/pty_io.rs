...
fn main() {
    ...
    for _ in 0..ITERATIONS {
        endpoint
            .write_all_bounded(black_box(PAYLOAD), Duration::from_secs(2))
            .expect("benchmark write");

        let mut iteration_received = 0;
        while iteration_received < PAYLOAD.len() {
            match endpoint.read(&mut buffer).expect("benchmark read") {
    ...
    let _ = endpoint.terminate(TerminationPolicy::new(
        Duration::from_millis(100),
        Duration::from_secs(2),
    ));

    println!("[seyal pty benchmark] workload=m001-pty-roundtrip");
