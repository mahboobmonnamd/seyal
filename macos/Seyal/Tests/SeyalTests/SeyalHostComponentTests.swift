...
final class SeyalHostComponentTests: XCTestCase {
    func testApplicationRootABIMatchesPublishedHeader() {
        ...
        XCTAssertEqual(MemoryLayout<SeyalAppChrome>.size, 28)
        XCTAssertEqual(MemoryLayout<SeyalAppShell>.size, 64)
        XCTAssertEqual(MemoryLayout<SeyalAppRow>.size, 56)
        XCTAssertEqual(MemoryLayout<SeyalAppTheme>.size, 52)
        XCTAssertEqual(MemoryLayout<SeyalAppChromeSurface>.size, 16)
        ...
        XCTAssertEqual(dark.intent_receded, UInt16(SEYAL_APP_MATERIAL_FROSTED))
        let reduced = seyal_app_theme(0, UInt16(SEYAL_APP_THEME_REDUCE_TRANSPARENCY | SEYAL_APP_THEME_REDUCE_MOTION))
        XCTAssertEqual(reduced.intent_receded, UInt16(SEYAL_APP_MATERIAL_TONAL))
        XCTAssertEqual(reduced.focus_duration, 0)
        let depthHandle = seyal_app_create()
        let left = seyal_app_chrome_surface(depthHandle, UInt16(SEYAL_APP_SURFACE_LEFT), 0, 0)
        XCTAssertEqual(left.depth, UInt16(SEYAL_APP_DEPTH_RECEDED))
        XCTAssertEqual(seyal_app_destroy(depthHandle), 0)
    }
    ...
    func testNativeTestsConsumeRustApplicationRootFixturesOnly() {
        ...
        XCTAssertEqual(chrome.center_surface, 0)
        XCTAssertEqual(chrome.reserved & UInt32(SEYAL_APP_CHROME_LEFT_VISIBLE), UInt32(SEYAL_APP_CHROME_LEFT_VISIBLE))
        XCTAssertEqual(chrome.reserved & UInt32(SEYAL_APP_CHROME_INSPECTOR_VISIBLE), UInt32(SEYAL_APP_CHROME_INSPECTOR_VISIBLE))
        XCTAssertEqual(chrome.reserved & UInt32(SEYAL_APP_CHROME_TAB_STRIP_VISIBLE), UInt32(SEYAL_APP_CHROME_TAB_STRIP_VISIBLE))
        ...
        var center = SeyalAppAction()
        center.version = UInt16(SEYAL_APP_ABI_VERSION)
        center.size = UInt16(MemoryLayout<SeyalAppAction>.size)
        center.kind = UInt16(SEYAL_APP_ACTION_SET_CENTER_SURFACE.rawValue)
        center.reserved = 1
        XCTAssertEqual(seyal_app_apply(handle, &center), 0)
        XCTAssertEqual(seyal_app_chrome(handle).center_surface, 1)
        var select = SeyalAppAction()
        select.version = UInt16(SEYAL_APP_ABI_VERSION)
        select.size = UInt16(MemoryLayout<SeyalAppAction>.size)
        select.kind = UInt16(SEYAL_APP_ACTION_SELECT_AGENT.rawValue)
        let missing = Array("missing-agent".utf8)
        missing.withUnsafeBufferPointer { buffer in
            select.payload = buffer.baseAddress
            select.payload_len = UInt32(buffer.count)
            XCTAssertNotEqual(seyal_app_apply(handle, &select), 0)
        }
        // Selection fails closed: center mode stays Agents, no selected agent row flag.
        let stillAgents = seyal_app_chrome(handle)
        XCTAssertEqual(stillAgents.center_surface, 1)
        let agentAfterFail = seyal_app_chrome_row(handle, UInt16(SEYAL_APP_ROW_AGENT), 0)
        XCTAssertEqual(agentAfterFail.flags & UInt16(SEYAL_APP_ROW_SELECTED), 0)
