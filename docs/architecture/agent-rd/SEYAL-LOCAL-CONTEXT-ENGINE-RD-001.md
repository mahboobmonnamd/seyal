...
- dirty-file content hash change;
...
```text
safe to share: identical source blob → chunk/symbol/embedding derived from that blob

must be worktree-scoped: dirty state, diff, branch, selection, bundle, user-pinned context
```
...
- secret-like file that must not enter a bundle;
...
The OSS baseline passes when retained evaluation tasks can deterministically explain why context was selected, stale/unauthorized items are excluded, dirty worktrees remain isolated, and optional semantic retrieval can be disabled without breaking correctness.

Reject any design that requires a vector service, cloud model, commercial account, full-repository prompt, or terminal hot-path callback for ordinary local operation.

## ADR/spec before implementation

- Local Context ownership/provenance/invalidation ADR after cache R&D #53 aligns fingerprints.
- ContextItem/ContextBundle/SelectionTrace behavior spec.
