...
fn retained_history(terminal: &TerminalState) -> Vec<(LineId, Vec<Cell>)> {
    let visible_ids: Vec<_> = (0..terminal.rows()).filter_map(|row| terminal.line_id(row)).collect();
    let max_id = visible_ids.iter().copied().max().unwrap_or(LineId(1));
    terminal
        .primary_history_range(LineId(1), max_id, 8_192)
        .into_iter()
...
fn main() {
    ...
    let canonical = retained_history(&terminal);
    ...
    let after_resize = retained_history(&terminal);
    assert!(retained_rows_unchanged(&before_resize, &after_resize), "pre-existing retained history rows must keep exact LineId + Cell payload across resize");
