...
Proposed gates may be collected when harness_status=ready. Collection
does not evaluate a release PASS/FAIL (the validator rejects proposed
gates).
"""
...
def self_test() -> None:
    ...
    for name, family in families.items():
        ...
        if name not in HISTORY_GATES and family.get("gate_status") != "proposed":
...
def collect_pty_to_terminal_state(gate: str, dest: Path, sha: str) -> str:
...
def collect_gate(gate: str, *, allow_history: bool) -> None:
    ...
    if gate != "pty_to_terminal_state":
        raise SystemExit(f"{gate} is inventoried but has no five-cohort collector yet")
    sha = git_sha()
    ...
    log.write_text(collect_pty_to_terminal_state(gate, candidate, sha), encoding="utf-8")
