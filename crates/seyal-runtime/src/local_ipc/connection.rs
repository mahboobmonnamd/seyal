...
impl LocalIpcServer {
            ...
            match self.listener.accept() {
                Ok((stream, _)) => {
                    ...
                    self.connections.insert(token, Connection {
                        stream, state: ConnectionState::AwaitHello, read_buf: Vec::with_capacity(4096),
                        mandatory: VecDeque::new(), after_display: VecDeque::new(), queued_control_bytes: 0,
                        display_inflight: None, pending_display: None, display_generation: 0,
                    });
    ...
    pub fn service_read(&mut self, token: u64, hangup: bool) -> Vec<ServerEvent> {
        ...
        while events.len() < MAX_FRAMES_PER_READINESS {
            ...
            if remaining == 0 { events.push(ServerEvent::FramingError { token }); self.close_with_event(token, &mut events); return events; }
            match fd_transfer::recv_with_fd(connection.stream.as_raw_fd(), &mut chunk[..READ_CHUNK_BYTES.min(remaining)]) {
                ...
                Ok((count, RecvFd::None)) => {
                    ...
                    if drain_frames(connection, token, &mut events).is_err() { self.close_with_event(token, &mut events); return events; }
                }
    ...
    /// Admission is atomic with respect to socket I/O because enqueue methods
    /// never flush; only a later reactor writable event calls `service_write`.
    ...
    pub fn wants_write(&self, token: u64) -> bool {
        self.connections.get(&token).is_some_and(|c| !c.mandatory.is_empty() || c.display_inflight.is_some() || c.pending_display.is_some() || !c.after_display.is_empty())
    }
...
fn drain_frames(connection: &mut Connection, token: u64, events: &mut Vec<ServerEvent>) -> Result<(), FramingCutError> {
    while events.len() < MAX_FRAMES_PER_READINESS {
        ...
        let header = match FrameHeader::decode(&connection.read_buf[..HEADER_LEN]) { Ok(h) => h, Err(_) => { events.push(ServerEvent::FramingError { token }); return Err(FramingCutError); } };
        let total = HEADER_LEN.checked_add(header.payload_len as usize).ok_or(FramingCutError)?;
        if connection.read_buf.len() < total { return Ok(()); }
...
fn flush_outbound(connection: &mut Connection) -> io::Result<()> {
    ...
    loop {
        ...
        if item.frame_index >= item.batch.frames.len() { connection.display_inflight = None; continue; }
        ...
        match flush_bytes(connection.stream.as_raw_fd(), &frame, &mut item.sent)? {
            ...
            FlushProgress::Progress => {
                if item.sent == frame.len() {
                    item.frame_index += 1; item.sent = 0;
                    if item.frame_index >= item.batch.frames.len() { connection.display_inflight = None; continue; }
                }
                return Ok(());
            }
...
fn flush_bytes(socket: RawFd, bytes: &[u8], sent: &mut usize) -> io::Result<FlushProgress> {
    ...
    match fd_transfer::send_with_fd(socket, &bytes[*sent..], None) {
        ...
        Ok(count) => {
            ...
            *sent += count; Ok(FlushProgress::Progress)
        }
        Err(e) if e.kind() == io::ErrorKind::WouldBlock => Ok(FlushProgress::WouldBlock),
        Err(e) => Err(e),
    }
}
...
mod tests {
    ...
    fn connection() -> Connection {
        Connection { stream: UnixStream::pair().unwrap().0, state: ConnectionState::Attached, read_buf: Vec::new(), mandatory: VecDeque::new(), after_display: VecDeque::new(), queued_control_bytes: 0, display_inflight: None, pending_display: None, display_generation: 1 }
    }
    #[test]
    fn pending_slot_prevents_unbounded_display_history() {
        let mut c = connection();
        assert_eq!(c.try_queue_delta(encode_delta(&sample(2), 1).unwrap()), DeltaEnqueueResult::Queued);
        assert_eq!(c.try_queue_delta(encode_delta(&sample(3), 2).unwrap()), DeltaEnqueueResult::NeedSnapshot);
        assert_eq!(c.pending_display.as_ref().unwrap().generation, 2);
    }
    #[test]
    fn snapshot_supersedes_not_started_pending_state() {
        let mut c = connection();
        c.queue_snapshot(encode_snapshot(&sample(7)).unwrap());
        c.queue_snapshot(encode_snapshot(&sample(9)).unwrap());
        assert_eq!(c.display_generation, 9);
        assert_eq!(c.pending_display.as_ref().unwrap().generation, 9);
    }
}
