...
fn assert_atomic_error(cache: &DisplayCache, before: &DisplayCache, result: Result<(), seyal_runtime::display::DisplayError>) {
    if result.is_err() {
        assert_eq!(cache, before, "rejected display batch partially mutated client cache");
    }
}
...
fuzz_target!(|data: &[u8]| {
    ...
    // Keep generated valid states deliberately small. The arbitrary-byte decoder
    // target covers hostile lengths/geometry; this target concentrates mutation
    // budget on generation continuity, chunk ordering and atomic cache updates.
