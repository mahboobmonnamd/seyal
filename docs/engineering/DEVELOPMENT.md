...
4. Any request to implement/fix/finish/code a specific GitHub Issue must enter `.agents/skills/implement-issue/SKILL.md`. Resolve the authenticated GitHub login, fresh-read assignees, and complete the assignment write so the current implementer is the sole GitHub assignee before planning or production work. Do not skip the write, and do not use `gh issue edit --add-assignee`. Assigned-to-other, multiple-assignee, identity-unavailable, or assignment-write-failed cases stop as `BLOCKED`.
