...
- [`SPEC-013-M005-CONTEXT-BUNDLE-SELECTION-TRACE.md`](SPEC-013-M005-CONTEXT-BUNDLE-SELECTION-TRACE.md) — **Proposed under #854; ADR-013 accepted:** provider-neutral Local Context Engine contract for provenance-bound `ContextItem`s, deterministic eligibility/authority ordering, immutable dependency-tracked `ContextBundle`, policy-safe `SelectionTrace`, filesystem/worktree/symlink/submodule and LSP generation provenance, invalidation, bounded retrieval and terminal hot-path isolation. MemoryRecord lifecycle remains separately owned by SPEC-012/#847.
