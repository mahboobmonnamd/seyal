...
2. Enter/Tab/Backspace with no Alt/Control use the legacy table, including Shift-Tab. With flag 1 and Alt or Control they use `CSI code;m u` (codes 13/9/127), but no release is emitted because flag 8 is absent. When flag 1 is absent, their legacy modifier table applies. Enter with no modifier remains CR even when flags=3.
