    ...
    fn reconstruct(&self) -> Vec<(LineId, Vec<Cell>)> {
        ...
        for segment in &self.segments {
            for meta in &segment.metas {
                ...
                rows.push((meta.id, segment.cells[start..end].to_vec()));
            }
        }
        rows
    }
...
fn main() {
    ...
    segmented.finish();
    let reconstructed = segmented.reconstruct();
    assert_eq!(reconstructed, canonical, "segmented storage must preserve fixture cells and IDs exactly");
    ...
    let styled_cells = canonical.iter().flat_map(|(_, cells)| cells).filter(|cell| cell.style != Cell::default().style).count();
    let utf8_cells = canonical.iter().flat_map(|(_, cells)| cells).filter(|cell| !cell.character.is_ascii()).count();
    ...
    assert_eq!(before_resize, after_resize, "M001 retained history payload must remain stable across current resize behavior");
    ...
    assert_eq!(history(&terminal), before_resize, "alternate screen must not contaminate primary retained history");
