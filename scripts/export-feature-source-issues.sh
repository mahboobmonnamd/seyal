...
export_repo() {
  local repo="$1" name="$2"
  local raw="$TMP_DIR/$name-issues-raw.json" rows="$TMP_DIR/$name-issues.ndjson" out="$OUT_DIR/$name-issues.json"

  echo "Exporting $repo ..."
  gh issue list --repo "$repo" --state all --limit 10000 \
    --json number,title,body,state,stateReason,url,author,labels,assignees,milestone,createdAt,updatedAt,closedAt > "$raw"
  ...
  [[ "$expected" == "$exported" ]] || { echo "error: $repo issue count mismatch: expected=$expected exported=$exported" >&2; exit 1; }
  ...
  while IFS= read -r issue_json; do
    ...
    comments_json="$(gh api --paginate "repos/$repo/issues/$number/comments?per_page=100" \
      --jq '.[] | {id: .id, url: .html_url, body: (.body // ""), createdAt: .created_at, updatedAt: .updated_at, author: {login: (.user.login // "unknown")}}' | jq -s '.')"
    ...
    if (( index % 25 == 0 || index == exported )); then echo "  exported $index/$exported issues ($total_comments comments)"; fi
  done < <(jq -c '.[]' "$raw")

  jq -s '.' "$rows" > "$out"
  final_count="$(jq 'length' "$out")"
  [[ "$final_count" == "$expected" ]] || { echo "error: $repo final export count mismatch: expected=$expected final=$final_count" >&2; exit 1; }
...
rill_inventory_total="$(gh issue list --repo mahboobmonnamd/RILL --state all --limit 10000 --label inventory --json number --jq 'length')"
rill_feature_rows="$(gh issue list --repo mahboobmonnamd/RILL --state all --limit 10000 --label inventory --json title --jq '[.[] | select(.title | test("^F-[0-9]+ "))] | length')"
rill_unique_feature_ids="$(gh issue list --repo mahboobmonnamd/RILL --state all --limit 10000 --label inventory --json title --jq '[.[] | .title | select(test("^F-[0-9]+ ")) | split(" ")[0]] | unique | length')"
