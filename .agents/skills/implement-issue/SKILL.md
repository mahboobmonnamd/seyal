...
9. After the verified claim comment, set the Issue body field `State` to **In Progress** and any linked Project item to the matching status. Verify this lifecycle update; if it fails or is ambiguous, stop and report the reserved branch.
10. Create the isolated worktree from the verified exact branch only after the claim comment and lifecycle update are present. The branch remains reserved through implementation and review.
