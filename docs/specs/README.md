...
- [`SPEC-007-M001-BLOCKS.md`](SPEC-007-M001-BLOCKS.md) — **Proposed for M001 Pass 8 / Issue #708:** Runtime/workspace-owned minimal `TerminalActivity` Block metadata, stable `BlockId → ExecutionId` association, immutable `BeforeLine(LineId)` primary-history anchor, monotonic `Current → Completed` lifecycle, capability-gated read-only local `BlockState` projection, raw-terminal failure fallback and strict no-hot-path/no-copied-transcript constraints. Production implementation remains blocked until Pass 7 exits pass and this specification is independently accepted.
