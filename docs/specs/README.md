...
- [`SPEC-004-M001-LOCAL-ATTACHMENT-PROJECTION.md`](SPEC-004-M001-LOCAL-ATTACHMENT-PROJECTION.md) — **Accepted for M001 Pass 5:** Candidate-D versioned local binary control/input and snapshot/delta display-state transport, same-user attachment authority, generation/resync behavior, hostile-client/resource constraints and measured production-path acceptance. Its semantic-key and generation-correlated resize extensions are governed by accepted SPEC-006 / PR #703; Pass 7 production implementation remains independently gated by #706 / PR #707.
...
- [`SPEC-008-M001-DETACH-RECONNECT.md`](SPEC-008-M001-DETACH-RECONNECT.md) — **Proposed for M001 Pass 9 / Issue #717:** same-`RuntimeId`/same-`ExecutionId` survival across normal GUI detach and abrupt GUI death, fresh attachment/controller authority on reconnect, bounded controller-race recovery, current-state reconstruction without PTY replay, detached-output/exit semantics, Pass 8 Block continuity, and repeated lifecycle/security/performance/resource proof. Production implementation remains `NOT_READY` until Pass 8 exits and SPEC-008 is accepted.
