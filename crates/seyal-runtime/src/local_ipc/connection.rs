...
impl LocalIpcServer {
    ...
    pub fn presentation_generation(&self, token: u64) -> Option<u64> {
        self.connections.get(&token).map(|c| c.display_generation)
    }
    ...
    pub fn service_read(&mut self, token: u64, hangup: bool) -> Vec<ServerEvent> {
        ...
        if hangup {
            if self
                .connections
                .get(&token)
                .is_some_and(|c| !c.read_buf.is_empty())
            {
                events.push(ServerEvent::FramingError { token });
            }
            self.close_with_event(token, &mut events);
    ...
    /// Atomically admits the `Attached` control frame and its initial snapshot
    /// into the bounded connection state before Runtime publishes attachment or
    /// controller authority. A later transport error closes the connection.
    pub fn enqueue_attach_transaction(
        ...
        if connection.display_inflight.is_some() || connection.pending_display.is_some() {
            return Err(io::Error::other("attach transaction requires empty display queue"));
        }
        ...
        if test_fault::take(FaultPoint::AttachFlush) {
            return Err(io::Error::other("injected attach flush failure"));
        }
...
mod tests {
    ...
    fn pending_slot_prevents_unbounded_display_history() {
        ...
        assert_eq!(
            connection.try_queue_delta(encode_delta(&sample(2), 1).unwrap()),
            DeltaEnqueueResult::Queued
        );
        assert_eq!(
            connection.try_queue_delta(encode_delta(&sample(3), 2).unwrap()),
    ...
    fn snapshot_supersedes_not_started_pending_state() {
        ...
        connection.queue_snapshot(encode_snapshot(&sample(7)).unwrap());
        connection.queue_snapshot(encode_snapshot(&sample(9)).unwrap());
        assert_eq!(connection.display_generation, 9);
        assert_eq!(connection.pending_display.as_ref().unwrap().generation, 9);
    }
}
