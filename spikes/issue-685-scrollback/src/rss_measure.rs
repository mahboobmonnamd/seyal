...
fn build_segmented(units: usize) -> Store {
    ...
    while produced < units {
        let len = logical_line_len(line, units - produced);
        if !current.atoms.is_empty() && current.atoms.len() + len > SEGMENT_TARGET_UNITS {
        ...
        let start = current.atoms.len() as u32;
        for i in 0..len {
            current.atoms.push(atom(produced + i));
        }
        current.lines.push((line as u64 + 1, start, len as u32));
        produced += len;
        line += 1;
    }
...
fn run_worker(candidate: &str, units: usize, executions: usize) -> (usize, usize) {
    ...
    let objects = fields.next().expect("objects").parse().expect("numeric objects");
    (rss, objects)
}
...
fn main() {
    ...
    if args.get(1).map(String::as_str) == Some("worker") {
        ...
        let units = args.get(3).expect("units").parse().expect("numeric units");
        let executions = args.get(4).expect("executions").parse().expect("numeric executions");
    ...
    report.push_str(&format!("Linux worker baseline VmRSS: {baseline} KiB. Delta values subtract this process/runtime baseline. Measurements are allocator/OS observations for the isolated spike model, not production RSS promises.\n\n"));
    ...
    for units in [10_000usize, 100_000, 1_000_000] {
        for candidate in candidates {
            ...
            report.push_str(&format!("| {candidate} | {units} | {rss} | {} | {objects} |\n", rss.saturating_sub(baseline)));
        }
    }
    ...
    for executions in [1usize, 10, 50, 100] {
        for candidate in candidates {
            ...
            report.push_str(&format!("| {candidate} | {executions} | {rss} | {delta} | {} | {objects} |\n", delta / executions));
        }
    }
